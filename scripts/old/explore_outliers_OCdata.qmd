...
```{r}
summary(oc_coarse_norm[,6:37])

```

### log10(x+1)

Definitely right-skewed distributions throughout. Let's transform using a log10(x+1) transformation (due to multiple 0s in data) and potentially box-cox transformation. I've never done the latter and I need to know if you should use the same lambda for all variables or not.

The result: data are still quite right-skewed.

```{r}
oc_coarse_normlog <- oc_coarse_norm |>
  mutate(across(c("appliances", "toys"), ~log1p(.x)))

summary(oc_coarse_normlog[,6:37]) ## still pretty right-skewed

# log_shap <- lapply(oc_coarse_normlog[,6:37], shapiro.test) ## sample size too large

ggplot(pivot_longer(oc_coarse_normlog[,c(1,6:37)], cols = !`Cleanup ID`), aes(x = value)) +
  geom_histogram(bins = 10) +
  facet_wrap(~name)

# outlier_values <- do.call("cbind",lapply(oc_coarse_normlog[,6:37],function(x) boxplot.stats(x)$stats)) ## returns 
...
## create list of outlier values (try) and get the minimum outlier per plastic item (min_outliers)
try <- outliers(oc_coarse_normlog[,6:37])
str(try) ## suggests lots of outliers...like, up to 21% for a single variable. 
sorted_try <- lapply(try,sort)
min_outliers <- as.data.frame(sapply(sorted_try,"[[",1)) |>
  rownames_to_column("name") |>
  setNames(c("name", "min_outlier"))
view(min_outliers) ## remember, these are the log10(x+1) transformed values

## view some qqplots of outliers
qqPlot(oc_coarse_normlog$`cigarettes & filters`)
qqPlot(oc_coarse_normlog$`cigarettes & filters`, ylim = c(0,4))
qqPlot(oc_coarse_normlog$`food wrappers & containers`)
qqPlot(oc_coarse_normlog$`balloons`)
qqPlot(oc_coarse_normlog$condoms, ylim = c(0,10))
...
```{r}
oc_coarse_recip1 <- oc_coarse_norm |>
  mutate(across(c("appliances", "toys"), ~1/sqrt(.x)))

summary(oc_coarse_recip1[,6:37]) ## still pretty right-skewed

ggplot(pivot_longer(oc_coarse_recip1[,c(1,6:37)], cols = !`Cleanup ID`), aes(x = value)) +
  geom_histogram(bins = 10) +
  facet_wrap(~name)

## view some qqplots of outliers
qqPlot(oc_coarse_recip1$`cigarettes & filters`)
qqPlot(oc_coarse_recip1$`cigarettes & filters`, ylim = c(0,4))
qqPlot(oc_coarse_recip1$`food wrappers & containers`)
qqPlot(oc_coarse_recip1$`balloons`)
qqPlot(oc_coarse_recip1$condoms, ylim = c(0,10))
```

### 1/x

```{r}
oc_coarse_recip2 <- oc_coarse_norm |>
  mutate(across(c("appliances", "toys"), ~1/.x))

summary(oc_coarse_recip2[,6:37]) ## still pretty right-skewed

ggplot(pivot_longer(oc_coarse_recip2[,c(1,6:37)], cols = !`Cleanup ID`), aes(x = value)) +
  geom_histogram(bins = 10) +
  facet_wrap(~name)

## view some qqplots of outliers
qqPlot(oc_coarse_recip2$`cigarettes & filters`)
qqPlot(oc_coarse_recip2$`cigarettes & filters`, ylim = c(0,4))
qqPlot(oc_coarse_recip2$`food wrappers & containers`)
qqPlot(oc_coarse_recip2$`balloons`)
qqPlot(oc_coarse_recip2$condoms, ylim = c(0,10))
```

### 1/(x\^2)

```{r}
oc_coarse_recip3 <- oc_coarse_norm |>
  mutate(across(c("appliances", "toys"), ~1/((.x)^2)))

summary(oc_coarse_recip3[,6:37]) ## still pretty right-skewed

ggplot(pivot_longer(oc_coarse_recip3[,c(1,6:37)], cols = !`Cleanup ID`), aes(x = value)) +
  geom_histogram(bins = 10) +
  facet_wrap(~name)

## view some qqplots of outliers
qqPlot(oc_coarse_recip3$`cigarettes & filters`)
qqPlot(oc_coarse_recip3$`cigarettes & filters`, ylim = c(0,4))
qqPlot(oc_coarse_recip3$`food wrappers & containers`)
qqPlot(oc_coarse_recip3$`balloons`)
qqPlot(oc_coarse_recip3$condoms, ylim = c(0,10))
```

### Remove 2020 data

```{r}
## run parts 7-current first
str(oc_coarse_norm)
rm_20s_log <- oc_coarse_normlog |>
  dplyr::filter(`Cleanup Date` < "2020-01-01")
table(as.factor(rm_20s_log$`Cleanup Date`))

ggplot(pivot_longer(rm_20s_log[,c(1,6:37)], cols = !`Cleanup ID`), aes(x = value)) +
  geom_histogram(bins = 10) +
  facet_wrap(~name)

qqPlot(rm_20s_log$`cigarettes & filters`)
qqPlot(rm_20s_log$`cigarettes & filters`, ylim = c(0,4))
qqPlot(rm_20s_log$`food wrappers & containers`)
qqPlot(rm_20s_log$`balloons`)
qqPlot(rm_20s_log$condoms, ylim = c(0,10))

## try with 1/(x^2) data
rm_20s_recip3 <- oc_coarse_recip3 |>
  dplyr::filter(`Cleanup Date` < "2020-01-01")
table(as.factor(rm_20s_recip3$`Cleanup Date`))

ggplot(pivot_longer(rm_20s_recip3[,c(1,6:37)], cols = !`Cleanup ID`), aes(x = value)) +
  geom_histogram(bins = 10) +
  facet_wrap(~name)

qqPlot(rm_20s_recip3$`cigarettes & filters`)
qqPlot(rm_20s_recip3$`cigarettes & filters`, ylim = c(0,4))
qqPlot(rm_20s_recip3$`food wrappers & containers`)
qqPlot(rm_20s_recip3$`balloons`)
qqPlot(rm_20s_recip3$condoms, ylim = c(0,10))

```

### Is normalized data any good?

Viewing counts of cigarettes & filters from cleanups with the highest normalized counts of this category

```{r}
tester <- oc_coarse_counts |>
  dplyr::filter(`Cleanup ID` %in% c("106875", "23205", "6133", "82178", "5130"))
```

## Umap of 10 most collected plastics categories
